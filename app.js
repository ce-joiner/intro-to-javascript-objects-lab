...
game.party = [ pokemon[6], pokemon[150], pokemon[38], pokemon[145]]
...
game.catchPokemon = function (pokemonName) {
    ...
    const pokemonObj = pokemon.find(pokemon => pokemon.name.toLowerCase() === normalizedPokemonName);
        if (!pokemonObj) {
...
*/
const groupedPokemonByType = pokemon.reduce((acc, pokemon) => {
    if (!acc[pokemon.type]) {
      acc[pokemon.type] = [];
    }
